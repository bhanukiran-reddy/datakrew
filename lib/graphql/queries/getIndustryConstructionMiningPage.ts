...
const GET_INDUSTRY_CONSTRUCTION_MINING_PAGE = /* GraphQL */ `
query getIndustryConstructionMiningPage($uri: ID!) {
...
export const getIndustryConstructionMiningPage = cache(async (locale: string, uri: string) => {
  const response = await fetchGraphQL<{ industry: any }>(
    GET_INDUSTRY_CONSTRUCTION_MINING_PAGE,
    { uri }
  );
