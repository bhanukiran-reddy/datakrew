...
const GET_INDUSTRY_PUBLIC_TRANSPORT_PAGE = /* GraphQL */ `
query getIndustryPublicTransportPage($id: ID!) {
...
export const getIndustryPublicTransportPage = cache(async (locale: string, uri: string) => {
    const response = await fetchGraphQL<{ industry: any }>(
        GET_INDUSTRY_PUBLIC_TRANSPORT_PAGE,
        { id: uri }
    );
