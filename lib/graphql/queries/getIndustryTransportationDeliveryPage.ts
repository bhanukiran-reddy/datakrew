...
const GET_INDUSTRY_TRANSPORTATION_DELIVERY_PAGE = /* GraphQL */ `
query getIndustryTransportationDeliveryPage($id: ID!) {
  industry(id: $id, idType: URI) {
    seo {
      title
      metaDesc
      canonical
...
export const getIndustryTransportationDeliveryPage = cache(async (locale: string, uri: string) => {
    const response = await fetchGraphQL<{ industry: any }>(
        GET_INDUSTRY_TRANSPORTATION_DELIVERY_PAGE,
        { id: uri }
    );
