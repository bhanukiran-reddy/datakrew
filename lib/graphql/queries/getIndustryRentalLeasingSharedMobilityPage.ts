...
const GET_INDUSTRY_RENTAL_LEASING_SHARED_MOBILITY_PAGE = /* GraphQL */ `
query getIndustryRentalLeasingandSharedMobilityPage($uri: ID!) {
...
export const getIndustryRentalLeasingandSharedMobilityPage = cache(async (locale: string, uri: string) => {
  const response = await fetchGraphQL<{ industry: any }>(
    GET_INDUSTRY_RENTAL_LEASING_SHARED_MOBILITY_PAGE,
    { uri }
  );
