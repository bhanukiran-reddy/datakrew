...
import styles from "./rentalleasingsharedmobility.module.css";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const pageData = await getIndustryRentalLeasingandSharedMobilityPage(
        locale,
        "/industry/rental-leasing-shared-mobility/",
    ).catch(() => null);

    return buildMetadata(pageData?.seo || { title: "Rental, Leasing & Shared Mobility" }, locale, { path: 'industry/rental-leasing-shared-mobility' });
}

export default async function RentalLeasingSharedMobilityPage({ params }: Props) {
    const { locale } = await params;
    ...
    const pageData = await getIndustryRentalLeasingandSharedMobilityPage(
        ...
        console.warn(
            "[rental-leasing-shared-mobility] CMS fetch failed, using empty fallback:",
            err?.message ?? err,
        );
        return null;
    });
    ...
    const bannerData = {
        ...
        breadcrumbItems: [
            { label: "Home", href: "/" },
            { label: "Industry"},
            { label: "Rental, Leasing & Shared Mobility" },
        ],
        ctaButtons:
    ...
    return (
        <>
            <IndustryPageSchema
                slug="rental-leasing-shared-mobility"
