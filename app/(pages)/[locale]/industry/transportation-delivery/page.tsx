...
import styles from "./transportationdelivery.module.css";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const pageData = await getIndustryTransportationDeliveryPage(
        locale,
        "transportation-delivery",
    ).catch(() => null);

    return buildMetadata(pageData?.seo || { title: "Transportation and Delivery" }, locale, { path: 'industry/transportation-delivery' });
}

export default async function TransportationDeliveryPage({ params }: Props) {
    const { locale } = await params;
    ...
    const pageData = await getIndustryTransportationDeliveryPage(
        ...
        console.warn(
            "[transportation-delivery] CMS fetch failed, using empty fallback:",
            err?.message ?? err,
        );
        return null;
    });
    ...
    const bannerData = {
        ...
        breadcrumbItems: [
            { label: "Home", href: "/" },
            { label: "Industry"},
            { label: "Transportation and Delivery" },
        ],
        ctaButtons:
    ...
    return (
        <>
            ...
            <div>
                ...
                <FleetAgentsSection {...fleetAgentsDataDynamic} headerAlign="left" />
