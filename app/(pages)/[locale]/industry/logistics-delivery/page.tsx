...
import FeatureStackCard from "@/components/sections/reusable/FeatureStackCard/FeatureStackCard";
...
import { ultraSecureDesignData, completeYourIntelligenceData } from "./index";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const pageData = await getIndustryLogisticsPage(
        locale,
        "/industry/logistics-freight-services/",
    ).catch(() => null);

    return buildMetadata(pageData?.seo || { title: "Logistics & Delivery" }, locale, { path: 'industry/logistics-delivery' });
}
...
export default async function IndustryPage({ params }: Props) {
    ...
    const pageData = await getIndustryLogisticsPage(
        ...
        console.warn(
            "[logistics-delivery] CMS fetch failed, using empty fallback:",
            err?.message ?? err,
        );
        return null;
    });
    ...
    const textImageStatsSection = pageData?.textImageStatsSection || {};
    const oneDeviceDataDynamic = {
        title: textImageStatsSection.textImageStatsHeading || "",
        description: textImageStatsSection.textImageStatsDescription || "",
        image: textImageStatsSection.textImageStatsMedia?.node
            ? {
                url: textImageStatsSection.textImageStatsMedia.node.mediaItemUrl,
                alt: "Device showcase",
            }
            : { url: "", alt: "" },
        statsWidth: "narrow" as const,
        stats:
            textImageStatsSection.stats?.map((stat: any) => ({
                statsValue: stat.statsValue || "",
                statsDescription: stat.statsDescription || "",
            })) || [],
    };
    ...
    const ultraSecureDesignDataDynamic = {
        title:
            ...
            (ultraSecureDesignData as any).title,
        subtitle:
            staggeredGridSection.gridSectionDescription ||
            (ultraSecureDesignData as any).subtitle,
        columns: 3,
        cells:
            ...
            })) || (ultraSecureDesignData as any).cells,
    };
    ...
    return (
        <>
            ...
            <div>
                ...
                <OperationalCertaintySection {...operationalData} />
                <OneDevice {...oneDeviceDataDynamic} statsWidth="full" />
                {fleetAgentsDataDynamic.agents?.length > 0 && <FleetAgentsSection {...fleetAgentsDataDynamic} />}
