...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const pageData = await getIndustryPublicTransportPage(
        locale,
        "public-transport-schools",
    ).catch(() => null);

    return buildMetadata(pageData?.seo || { title: "Public Transport & Schools" }, locale, { path: 'industry/public-transport-schools' });
}

export default async function PublicTransportPage({ params }: Props) {
    const { locale } = await params;
    ...
    const pageData = await getIndustryPublicTransportPage(
        ...
        console.warn(
            "[public-transport] CMS fetch failed, using empty fallback:",
            err?.message ?? err,
        );
        return null;
    });
    ...
    const bannerData = {
        ...
        breadcrumbItems: [
            { label: "Home", href: "/" },
            { label: "Industry"},
            { label: "Public Transport & Schools" },
        ],
        ctaButtons:
    ...
    return (
        <>
            ...
            <div>
                ...
                <OperationalCertaintySection {...operationalData} className={styles.publictransport} />
