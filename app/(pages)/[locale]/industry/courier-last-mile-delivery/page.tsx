...
import styles from "./courierlastmiledelivery.module.css";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const pageData = await getIndustryCourierandLastMileDeliveryPage(
        locale,
        "/industry/courier-last-mile-delivery/",
    ).catch(() => null);

    return buildMetadata(pageData?.seo || { title: "Courier & Last Mile Delivery" }, locale, { path: 'industry/courier-last-mile-delivery' });
}

export default async function CourierLastMileDeliveryPage({ params }: Props) {
    const { locale } = await params;
    ...
    const pageData = await getIndustryCourierandLastMileDeliveryPage(
        ...
        console.warn(
            "[courier-last-mile] CMS fetch failed, using empty fallback:",
            err?.message ?? err,
        );
        return null;
    });
    ...
    return (
        <>
            ...
            <div>
                ...
                <OperationalCertaintySection {...operationalData} className={styles.courierlastmiledelivery} />
