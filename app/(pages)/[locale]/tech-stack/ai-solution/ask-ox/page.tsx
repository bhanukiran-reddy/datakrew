...
import styles from './askox.module.css';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    page = await getInfrastructureAskOXPage(locale, '/infrastructure/ask-ox').catch(() => null);

    return buildMetadata(page?.seo || { title: 'Ask OX' }, locale, { path: 'tech-stack/ai-solution/ask-ox' });
}

export const revalidate = 60;

export default async function AskOXPage({ params }: Props) {
    const { locale } = await params;
    ...
    } : (page?.sections?.filter((s) => s.type === 'contactStrip')?.[0]?.data || {});
    ...
    } : (page?.sections?.filter((s) => s.type === 'contactStrip')?.[1]?.data || {});
    ...
    return (
        <>
            <SoftwareApplicationPageSchema
                path="tech-stack/ai-solution/ask-ox"
                description={
            ...
            <FleetAgentsSection {...(fleetAgentsData as any)} id={navItems[6]?.id || "ouragents"} />
            <ContactStripSection {...(contactStripData1 as any)} id={navItems[8]?.id || "needacustomagent"} />
            <UltraSecureDesignSection {...(ultraSecureDesignData as any)} id={navItems[1]?.id || "needacustomagent"} columns={3} />
            <ProtectedByDesignSection {...(protectedByDesignData as any)} />
            <InstallItus
                id={navItems[2]?.id || "howitworks"}
            ...
            <BgTitleDescriptionSection {...(bgTitleDescriptionData as any)} id={navItems[4]?.id || "bgTitleDescription"} />
            <CompleteYourIntelligence {...(completeYourIntelligenceData as any)} id={navItems[3]?.id || "completeyourintelligence"} isAskOx />
